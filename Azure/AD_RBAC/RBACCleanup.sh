...
OBJECTID=$(az ad user show --upn-or-object-id devopsbot@[yourdomain]].onmicrosoft.com --query objectId --output tsv)
az ad group member add --group K8Cluster-View --member-id $OBJECTID
az ad user delete  --upn-or-object-id $OBJECTID
OBJECTID=$(az ad user show --upn-or-object-id devopsbot2@d[yourdomain].onmicrosoft.com --query objectId --output tsv)
az ad group member add --group K8Cluster-View --member-id $OBJECTID
az ad user delete  --upn-or-object-id $OBJECTID
